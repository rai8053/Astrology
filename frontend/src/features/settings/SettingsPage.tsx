            ...
            <div ref={currencyRef} className="relative">
              ...
              >
                ...
                <span className="flex-1 text-left">
                  {REGIONAL_PRICING[detectedCountry]?.currency.code} ({REGIONAL_PRICING[detectedCountry]?.currency.symbol}) ΓÇö {REGIONAL_PRICING[detectedCountry]?.currency.locale}
                </span>
              ...
              {currencyOpen && (
                <div className="absolute top-full left-0 right-0 mt-1 z-50 bg-card border border-border rounded-xl shadow-lg max-h-64 overflow-y-auto">
                  {Object.entries(REGIONAL_PRICING).map(([code, cfg]) => (
                    ...
                    >
                      <span>{cfg.flag}</span>
                      <span>{cfg.currency.code} ({cfg.currency.symbol}) ΓÇö {cfg.currency.locale}</span>
                    </button>
                  ))}
                </div>
              )}
