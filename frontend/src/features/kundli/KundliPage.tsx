...
export function KundliPage() {
  ...
  const mutation = useMutation({
    mutationFn: (data: BirthDetails) => api.post<VedicProfile>('/api/astrology/vedic-profile', { ...data, language }),
    onSuccess: () => setErrors({}),
  });
  ...
  useEffect(() => {
    if (viewMode === 'detailed' && profile && !detailedReport && !detailedLoading) {
      setDetailedLoading(true);
      api.post<{ detailedReport: string; requestKey?: string }>('/api/astrology/vedic-profile/detailed', {
        name: formData.name,
  ...
  }, [viewMode, profile, detailedReport, detailedLoading, language, formData]);
        ...
        >
          {mutation.isPending ? (
            ...
            <motion.div variants={containerVariants} initial="hidden" animate="visible">
              {viewMode === 'detailed' && detailedReport ? (
                <motion.div variants={itemVariants}>
                  <PremiumCard glass glow>
                    ...
                    <div
                      className="detailed-report prose prose-sm max-w-none text-ink/80 dark:text-parchment/80 leading-relaxed"
                ...
                <PremiumCard glass className="flex items-center justify-center py-24">
                  <div className="text-center">
                    <motion.div animate={{ rotate: 360 }} transition={{ duration: 2, repeat: Infinity, ease: 'linear' }}>
                      <Loader2 className="w-10 h-10 text-gold mx-auto mb-4" />
                    </motion.div>
                    <p className="text-sm text-ink/50 dark:text-parchment/50">{t('kundli.generatingDetailed')}</p>
                  </div>
                </PremiumCard>
              ) : (
                <>
